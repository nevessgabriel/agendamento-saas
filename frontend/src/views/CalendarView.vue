...
<template>
  <div class="calendar-layout">
    <div class="header-controls">
        ...
        <router-link to="/dashboard" class="btn-back">Voltar ao Dashboard</router-link>
    </div>
        ...
        <div v-for="time in timeSlots" :key="time" class="time-row">
            ...
            <div class="slot-content">
                <div v-if="getAppointmentForSlot(time)" class="appointment-card">
                    <span>
                        <strong>{{ getAppointmentForSlot(time).start_time.substring(11,16) }}</strong> - 
                        {{ getAppointmentForSlot(time).client_name }} 
                        ({{ getAppointmentForSlot(time).service_name }})
                    </span>
                    <button @click="handleCancel(getAppointmentForSlot(time).id)" class="btn-xs">X</button>
                </div>
        ...
        <div class="modal">
            ...
            <form @submit.prevent="handleBooking">
                <div class="form-group">
                    <label>Cliente</label>
                    <input v-model="form.clientName" required placeholder="Nome do Cliente"/>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select v-model="form.serviceId" required>
                        <option v-for="svc in serviceStore.services" :value="svc.id" :key="svc.id">
